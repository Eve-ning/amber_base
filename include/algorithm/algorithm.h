...
namespace algorithm
{
	...
	template <typename T>
    doublev offsetDiff(ObjV<T> const* objV) {
	...
	template <typename T>
    QSharedPointer<ObjV<T>> copyDelay(ObjV<T> const* obj_v,
                                        doublev offset_v,
                                        bool include) {
        // <0>   <1>   <2> IN
        //  0     2     4
    ...
    doublev copy_subd_by(doublev offset_v,
                         uint subdivisions,
                         bool include) {
        // [0]   [2]   [4] IN
        // [0][1][2][3][4] OUT
	...
	template <typename T>
    QSharedPointer<ObjV<T>> copySubdBy(doublev offset_v,
                                          const T& obj_define,
                                          uint subdivisions,
                                          bool include) {
        // [0]   [2]   [4] IN
	...
	template <typename T>
    QSharedPointer<ObjV<T>> copySubdBy(ObjV<T> const* obj_v,
                                          uint subdivisions,
                                          bool include) {
        // <0>   <1>   <2> IN
        //  0     2     4
		...
		//  0     2     4
		// [0][1][2][3][4]
        auto offset_v = copySubdBy(obj_v->getOffsetV(true), subdivisions, true);
		...
		// <0><0><2><2><4>
		//  0  1  2  3  4
        auto output = copyDelay(obj_v, offset_v, include);
